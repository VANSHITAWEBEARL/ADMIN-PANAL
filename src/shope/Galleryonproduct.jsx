...
import { FaPlus, FaTimes, FaShoppingBag, FaRedo, FaHeart, FaEye } from "react-icons/fa";
    ...
    <div>
      ...
      {modalOpen && selectedProduct && (
        <div className="fixed inset-0 z-50 bg-black bg-opacity-90 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg w-full max-w-6xl h-full max-h-screen overflow-auto">
            <div className="flex justify-end p-4">
              <button 
                onClick={closeModal}
                className="text-gray-500 hover:text-[#fe9995] text-2xl"
              >
                <FaTimes />
              </button>
            </div>
            ...
            <div className="flex flex-col md:flex-row p-6">
              ...
              <div className="w-full md:w-1/2 p-4">
                ...
                <div className="flex justify-center mt-4 gap-2">
                  {selectedProduct.icons.map((icon, index) => (
                    <img
                      key={index}
                      src={icon}
                      alt={`Thumbnail ${index + 1}`}
                      className="w-16 h-16 object-cover border border-gray-200 cursor-pointer hover:border-[#fe9995]"
                    />
                  ))}
                </div>
              </div>
              ...
              <div className="w-full md:w-1/2 p-4">
                ...
                <div className="text-[#fe9995] my-3 text-lg">☆☆☆☆☆</div>
                
                <p className="text-2xl font-medium text-gray-800 mb-4" style={{ fontFamily: 'Playfair Display , italic' }}>
                  {selectedProduct.price}
                </p>
                
                <p className="text-gray-600 mb-6">
                  {selectedProduct.description} Fusce vitae feugiat enim, id fermentum magna. 
                ...
                <button className="bg-[#fe9995] text-white py-3 px-8 rounded-full font-medium hover:bg-opacity-90 transition-all mb-6">
                  ADD TO CART
                </button>
                
                <div className="border-t border-gray-200 pt-4">
                  <p className="text-sm text-gray-600">
                    <span className="font-medium">Category:</span> Beauty, Organic
                  </p>
                  <p className="text-sm text-gray-600 mt-2">
                    <span className="font-medium">Tags:</span> Natural, Cosmetic, Organic
                  </p>
                </div>
              </div>
            </div>
            ...
            {products.map((product) => (
              <div key={product.id} className={`${viewMode === 'list' ?
                ...
                <div className={`${viewMode === 'list' ? 'text-left flex-1' : 'text-center'
                  ...
                  {/* Decorative Icon Buttons - Now clickable to open modal */}
                  <div className="flex justify-center gap-3 mb-2">
                    {product.icons.map((icon, i) => (
                      <button
                        key={i}
                        onClick={() => openModal(product)}
                        className="w-10 h-10 flex items-center justify-center rounded-full bg-gray-100 hover:bg-[#fe9995] transition"
                      >
